...
CREATE OR REPLACE PACKAGE PKG_PR_UTIL IS
  ...
  -- Delete payroll rate matrix for /rate/matrix
  FUNCTION DELETE_PR_RATE_MATRIX(v_be_id IN BE.BE_ID%TYPE, v_payer_id PAYER.PAYER_ID%TYPE, v_contr_id CONTR.CONTR_ID%TYPE, v_svc_name SVC.SVC_NAME%TYPE, v_rate_type_name BE_RATE.RATE_TYP_NAME%TYPE) RETURN NUMBER;

  FUNCTION SET_EXPORTED_DATE(v_pr_output_sk_array IN NUMBER_ARRAY, v_exported_date IN DATE) RETURN NUMBER;

  FUNCTION DELETE_TIMESHEET_DET(v_be_id IN TIMESHEET_DET.BE_ID%TYPE, v_visit_sk IN TIMESHEET_DET.VISIT_SK%TYPE) RETURN NUMBER;

  FUNCTION DELETE_TIMESHEET_SUMMARY(v_timesheet_summary_sk IN TIMESHEET_SUMMARY.TIMESHEET_SUMMARY_SK%TYPE) RETURN NUMBER;

END PKG_PR_UTIL;
/

CREATE OR REPLACE PACKAGE BODY PKG_PR_UTIL IS
  ...
  AS
    ...
    BEGIN
      ...
      SELECT
            ...
            T4.NET_PAY_AMT,
            T4.GROSS_PAY_AMT,
            PKG_PR_UTIL.GET_PAYROLL_PT_SVC(l_tss_sk) AS TIMESHEET_DET_CURSOR
  ...
  AS
    ...
    BEGIN
      ...
      l_sql := 'SELECT * FROM (SELECT ROWNUM ROW_NUMBER, COUNT(*) OVER() TOTAL_ROWS, R1.* FROM (
          SELECT * FROM (
              ...
              (SELECT
                      ...
                      T4.CHECK_DATE,
                      T4.GROSS_PAY_AMT,
                      PKG_PR_UTIL.GET_PAYROLL_PT_SVC(T1.TIMESHEET_SUMMARY_SK) AS TIMESHEET_DET_CURSOR
      ...
      IF v_svc_array IS NOT NULL AND v_svc_array.COUNT > 0 THEN
        ...
        l_services := 'AND PKG_PR_UTIL.SERVICES_EXISTS(T1.TIMESHEET_SUMMARY_SK, :b_svc_array) = 1';
      ...
      IF l_params.RATE_TYP_NAME IS NOT NULL THEN
        l_rate_typ_filter := 'AND PKG_PR_UTIL.RATE_TYPE_EXISTS(T1.TIMESHEET_SUMMARY_SK, :b_rate_typ) = 1';
      END IF;
  ...
  AS
    ...
    BEGIN
      ...
      IF l_params.SVC_NAME IS NULL THEN
        l_auth_svc := PKG_PR_UTIL.GET_AUTH_SVC(l_params.BE_ID, l_params.VISIT_SK, l_params.VISIT_DATE);
      ELSE
        l_auth_svc := l_params.SVC_NAME;
      END IF;

      IF l_params.RATE_SUB_TYP_NAME IS NULL THEN
        l_sub_rate_typ := PKG_PR_UTIL.GET_RATE_SUB_TYP(l_params.BE_ID, l_params.VISIT_DATE);
      ELSE
        l_sub_rate_typ := l_params.RATE_SUB_TYP_NAME;
      END IF;

      IF l_params.RATE_TYP_NAME IS NULL THEN
        l_rate_typ := PKG_PR_UTIL.GET_RATE_TYP_NAME(l_params.BE_ID, l_sub_rate_typ, l_params.PT_ID, l_params.VISIT_SK, l_params.VISIT_DATE);
      ELSE
  ...
  AS
    ...
    BEGIN
      ...
      IF l_params.SVC_NAME IS NULL THEN
        l_params.SVC_NAME := PKG_PR_UTIL.GET_AUTH_SVC(l_params.BE_ID, l_params.VISIT_SK, l_params.VISIT_DATE);
      END IF;

      IF l_params.RATE_SUB_TYP_NAME IS NULL THEN
        l_params.RATE_SUB_TYP_NAME := PKG_PR_UTIL.GET_RATE_SUB_TYP(l_params.BE_ID, l_params.VISIT_DATE);
      END IF;

      IF l_params.RATE_TYP_NAME IS NULL THEN
        l_params.RATE_TYP_NAME := PKG_PR_UTIL.GET_RATE_TYP_NAME(l_params.BE_ID, l_params.RATE_SUB_TYP_NAME, l_params.PT_ID, l_params.VISIT_SK, l_params.VISIT_DATE);
      END IF;
  ...
  AS
    ...
    BEGIN
      ...
      OPEN result_Cursor FOR
      SELECT PKG_PR_UTIL.GET_BE_RATE(l_params) AS BE_RATE_CURSOR,
              PKG_PR_UTIL.GET_STAFF_SUPPL_RATE(l_params) AS STAFF_SUPPL_RATE_CURSOR,
              PKG_PR_UTIL.GET_STAFF_ASSOC_RATE(l_params) AS STAFF_ASSOC_RATE_CURSOR,
              PKG_PR_UTIL.GET_PR_RATE_MATRIX(l_params) AS PR_RATE_MATRIX_CURSOR,
              T1.*
      FROM STAFF_RATE T1
      WHERE BE_ID = l_params.BE_ID
        AND SVC_NAME = l_params.SVC_NAME
  ...
  AS
    ...
    BEGIN
      ...
      v_rate_typ := COREDATA.PKG_PR_UTIL.GET_AUTH_RATE_TYP (v_be_id, v_visit_sk, visit_start_date);
  ...
  AS
    ...
    BEGIN
      ...
      V_RATE_SUB_TYP_NAME := PKG_PR_UTIL.HOLIDAY_RATE_SUB_TYP(v_be_id, visit_start_date);
      ...
      V_RATE_SUB_TYP_NAME := PKG_PR_UTIL.WEEKEND_RATE_SUB_TYP(v_be_id, visit_start_date);
  ...
  BEGIN
    ...
    WHERE BE_ID = v_be_id
      ...
      AND CONTR_ID = v_contr_id
      AND SVC_NAME = v_svc_name
      AND RATE_TYP_NAME = v_rate_type_name
      AND (TO_CHAR(REC_TERM_TMSTP, 'YYYY-MM-DD') = '9999-12-31' AND CURR_REC_IND = 1);
  
    RETURN SQL%ROWCOUNT;
  
  END DELETE_PR_RATE_MATRIX;
  ...
  BEGIN
    ...
    UPDATE PR_OUTPUT
    SET PR_EXPORT_DATE = v_exported_date
    WHERE PR_OUTPUT_SK IN (SELECT * FROM TABLE(v_pr_output_sk_array));

    RETURN SQL%ROWCOUNT;

  END SET_EXPORTED_DATE;
  ...
  BEGIN
    ...
    DELETE TIMESHEET_DET WHERE BE_ID = v_be_id AND VISIT_SK = v_visit_sk;

    RETURN SQL%ROWCOUNT;

  END DELETE_TIMESHEET_DET;
  ...
  BEGIN
    ...
    DELETE TIMESHEET_SUMMARY WHERE TIMESHEET_SUMMARY_SK = v_timesheet_summary_sk;

    RETURN SQL%ROWCOUNT;

  END DELETE_TIMESHEET_SUMMARY;
