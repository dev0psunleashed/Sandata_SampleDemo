...
BEGIN
  ...
  l_inv_sk_array := NUMBER_ARRAY (5,1);
  ...
  LOOP
    FETCH l_cursor INTO l_subm_typ,l_total_inv_billed,l_total_amt_billed,l_billed_hours;
  ...
  dbms_output.put_line ('*** START: PKG_BILLING_UTIL.GET_BRS_FOR_LOB ***');

  l_inv_sk_array := NUMBER_ARRAY (5,1);

  l_cursor := COREDATA.PKG_BILLING_UTIL.GET_BRS_FOR_LOB(l_inv_sk_array);
  ...
  LOOP
    FETCH l_cursor INTO l_lob_name,l_total_inv_billed,l_total_amt_billed,l_billed_hours;

    EXIT WHEN l_cursor%NOTFOUND;

    dbms_output.put_line ('l_cursor: l_lob_name = ' || l_lob_name);
    dbms_output.put_line ('l_cursor: l_total_inv_billed = ' || l_total_inv_billed);
    dbms_output.put_line ('l_cursor: l_total_amt_billed = ' || l_total_amt_billed);
    dbms_output.put_line ('l_cursor: l_billed_hours = ' || l_billed_hours || CHR(13) || CHR(10));
  ...
  dbms_output.put_line ('*** END: PKG_BILLING_UTIL.GET_BRS_FOR_LOB ***' || CHR(13) || CHR(10));
  ...
  LOOP
    ...
    IF l_subm_typ_cursor IS NOT NULL THEN
      ...
      LOOP
        FETCH l_subm_typ_cursor INTO l_subm_typ,l_total_inv_billed,l_total_amt_billed,l_billed_hours;
        ...
        dbms_output.put_line (CHR(9) || 'l_subm_typ_cursor: l_total_inv_billed = ' || l_total_inv_billed);
        dbms_output.put_line (CHR(9) || 'l_subm_typ_cursor: l_total_amt_billed = ' || l_total_amt_billed);
        dbms_output.put_line (CHR(9) || 'l_subm_typ_cursor: l_billed_hours = ' || l_billed_hours || CHR(13) || CHR(10));
