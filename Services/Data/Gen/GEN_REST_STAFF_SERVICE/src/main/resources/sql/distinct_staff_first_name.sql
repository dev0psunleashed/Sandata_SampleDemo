SELECT * FROM (SELECT ROWNUM ROW_NUMBER, COUNT(*) OVER() TOTAL_ROWS, r1.*
     FROM
        (SELECT DISTINCT T1.STAFF_SK,T1.STAFF_ID,T1.STAFF_FIRST_NAME,T1.STAFF_LAST_NAME,T6.LANG_NAME AS STAFF_PRMY_LANG,T1.STAFF_GENDER_TYP_NAME,
                  T3.STAFF_ADDR1,T3.STAFF_ADDR2,T3.STAFF_CITY,T3.STAFF_STATE,T3.STAFF_ZIP4,T2.STAFF_CERT_INSTITUTION
          FROM STAFF T1

          INNER JOIN (SELECT STAFF_ID,STAFF_CERT_RCVD_DATE,STAFF_CERT_EXPR_DATE,STAFF_CERT_INSTITUTION,REC_TERM_TMSTP,CURR_REC_IND
            FROM STAFF_CERT) T2
          ...
          LEFT JOIN (SELECT STAFF_ID,STAFF_PHONE,STAFF_PHONE_PRMY_IND,REC_TERM_TMSTP,CURR_REC_IND
            FROM STAFF_CONT_PHONE WHERE STAFF_PHONE_PRMY_IND = 1 AND
                  (TO_CHAR(REC_TERM_TMSTP, 'YYYY-MM-DD') = '9999-12-31' AND CURR_REC_IND = 1)) T4
          ON T1.STAFF_ID = T3.STAFF_ID

          LEFT JOIN (SELECT BE_ID,STAFF_ID,LANG_CODE,STAFF_LANG_PRMY_IND
            FROM STAFF_LANG WHERE STAFF_LANG_PRMY_IND = 1) T5
          ON T1.STAFF_ID = T5.STAFF_ID AND T1.BE_ID = T5.BE_ID

          INNER JOIN (SELECT LANG_CODE,LANG_NAME
            FROM LANG_LKUP) T6
          ON T5.LANG_CODE = T6.LANG_CODE

          WHERE T1.BE_ID = ? AND %s
          ...
          ORDER BY DECODE(STAFF_PRMY_LANG,'%s',1) ASC
    ) r1)
