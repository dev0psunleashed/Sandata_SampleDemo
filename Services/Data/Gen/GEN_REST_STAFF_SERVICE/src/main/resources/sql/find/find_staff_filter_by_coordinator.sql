SELECT * FROM
  (SELECT ROWNUM ROW_NUMBER, COUNT(*) OVER() TOTAL_ROWS, R1.* FROM
    (SELECT DISTINCT PKG_STAFF_UTIL.HIRE_DATE(T1.STAFF_ID, T2.BE_ID) AS ORIGINAL_STAFF_HIRE_DATE,
      T1.STAFF_POSITION_NAME,T1.STAFF_ID,T1.STAFF_SK,T1.STAFF_FIRST_NAME,T1.STAFF_LAST_NAME,T1.STAFF_DOB, T1.STAFF_EMPLT_STATUS_TYP_NAME,
      T1.STAFF_HIRE_DATE,T3.BE_ID_NUM AS STAFF_LOCATION_XWALK,T1.STAFF_LAST_HIRE_DATE,T1Addr.STAFF_ADDR1,T1Addr.STAFF_ADDR2,T1Addr.STAFF_CITY,
      T1Addr.STAFF_STATE,T1Addr.STAFF_PSTL_CODE,
      ...
      INNER JOIN (SELECT BE_SK,BE_ID,BE_NAME,BE_TYP,BE_PRMY_ADDR1,BE_PRMY_ADDR2,BE_PRMY_CITY,BE_PRMY_STATE,BE_PRMY_PSTL_CODE,
                    BE_PRMY_ZIP4,BE_PRMY_PHONE_1,REC_TERM_TMSTP,CURR_REC_IND
        FROM BE) T2
      ON T1.BE_ID = T2.BE_ID

      LEFT JOIN (SELECT BE_ID, BE_ID_TYP, BE_ID_QLFR, BE_ID_NUM
        FROM BE_ID_XWALK WHERE BE_ID_TYP = 'LOCATION' AND BE_ID_QLFR = 'CODE') T3
      ON T1.STAFF_LOC = T3.BE_ID

      -- Filter by given Coordinator (ADMIN_STAFF_ID)
      INNER JOIN (SELECT BE_ID,STAFF_ID,ADMIN_STAFF_ID,ADMIN_STAFF_STAFF_EFF_DATE,ADMIN_STAFF_STAFF_TERM_DATE
        FROM ADMIN_STAFF_STAFF_XREF
          WHERE (TO_CHAR(REC_TERM_TMSTP, 'YYYY-MM-DD') = '9999-12-31' AND CURR_REC_IND = 1)
            ...
            AND ADMIN_STAFF_ID = 'coord2@biz.org') ASSX
      ON T1.BE_ID = ASSX.BE_ID AND T1.STAFF_ID = ASSX.STAFF_ID

      WHERE T1.BE_ID = '1'
        AND T1.STAFF_POSITION_NAME = 'HHA'
        AND UPPER(T1.STAFF_LAST_NAME) LIKE 'RUTGOS%'
    ...
    ORDER BY UPPER(T1.STAFF_LAST_NAME) DESC) R1)
