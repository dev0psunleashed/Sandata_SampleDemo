CREATE OR REPLACE TYPE AR_TXN_LKUP_T AS OBJECT (
AR_TXN_LKUP_SK	NUMBER(38),
REC_CREATE_TMSTP DATE,
REC_UPDATE_TMSTP DATE,
REC_EFF_TMSTP DATE,
REC_TERM_TMSTP DATE,
REC_CREATED_BY	VARCHAR2(50 BYTE),
REC_UPDATED_BY	VARCHAR2(50 BYTE),
CHANGE_REASON_MEMO	VARCHAR2(255 BYTE),
CURR_REC_IND	NUMBER(1),
CHANGE_VERSION_ID	NUMBER(38),
AR_TXN_CODE	VARCHAR2(20 BYTE),
AR_TXN_NAME	VARCHAR2(100 BYTE),
AR_TXN_DESC	VARCHAR2(250 BYTE),
AR_TXN_SIGN_QLFR	VARCHAR2(50 BYTE),
AR_TXN_CTGY_CODE	VARCHAR2(50 BYTE)
);
/
