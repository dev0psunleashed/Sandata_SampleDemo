CREATE OR REPLACE TYPE BE_CALENDAR_LKUP_T AS OBJECT (
BE_CALENDAR_LKUP_SK	NUMBER(38),
REC_CREATE_TMSTP DATE,
REC_UPDATE_TMSTP DATE,
REC_EFF_TMSTP DATE,
REC_TERM_TMSTP DATE,
REC_CREATED_BY	VARCHAR2(50 BYTE),
REC_UPDATED_BY	VARCHAR2(50 BYTE),
CHANGE_REASON_MEMO	VARCHAR2(255 BYTE),
CURR_REC_IND	NUMBER(1),
CHANGE_VERSION_ID	NUMBER(38),
BE_ID	VARCHAR2(50 BYTE),
CALENDAR_DATE DATE,
DAY_OF_WEEK	NUMBER(1),
DAY_IN_YEAR	NUMBER(3),
WEEKDAY_IND	NUMBER(1),
WEEKEND_IND	NUMBER(1),
NATIONAL_HOLIDAY_IND	NUMBER(1),
STATE_HOLIDAY_IND	NUMBER(1),
LOC_HOLIDAY_IND	NUMBER(1)
);
/
