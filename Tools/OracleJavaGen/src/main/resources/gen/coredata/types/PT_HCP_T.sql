CREATE OR REPLACE TYPE PT_HCP_T AS OBJECT (
PT_HCP_SK	NUMBER(38),
REC_CREATE_TMSTP DATE,
REC_UPDATE_TMSTP DATE,
REC_EFF_TMSTP DATE,
REC_TERM_TMSTP DATE,
REC_CREATED_BY	VARCHAR2(50 BYTE),
REC_UPDATED_BY	VARCHAR2(50 BYTE),
CHANGE_REASON_MEMO	VARCHAR2(255 BYTE),
CURR_REC_IND	NUMBER(1),
CHANGE_VERSION_ID	NUMBER(38),
BE_ID	VARCHAR2(50 BYTE),
PT_ID	VARCHAR2(50 BYTE),
HCP_NPI	VARCHAR2(10 BYTE),
PT_HCP_START_DATE DATE,
PT_HCP_END_DATE DATE,
PT_HCP_PROVIDER_TYP_NAME	VARCHAR2(50 BYTE),
PT_HCP_PROVIDER_TYP_NAME_OTHER	VARCHAR2(50 BYTE),
PT_HCP_SIG_IND	NUMBER(1)
);
/
